...
BEGIN
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'applications'
    AND COLUMN_NAME = 'initial_fee_paid';
  IF column_count = 0 THEN
    ALTER TABLE applications ADD COLUMN initial_fee_paid TINYINT(1) NOT NULL DEFAULT 0 AFTER message;
  END IF;
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'applications'
    AND COLUMN_NAME = 'initial_fee_paid_at';
  IF column_count = 0 THEN
    ALTER TABLE applications ADD COLUMN initial_fee_paid_at DATETIME NULL AFTER initial_fee_paid;
  END IF;
  ...
  IF column_count = 0 THEN
    ALTER TABLE students ADD COLUMN agent_id INT NULL AFTER user_id;
  END IF;
  ...
  WHERE TABLE_SCHEMA = DATABASE()
    ...
    AND REFERENCED_COLUMN_NAME = 'id';
  IF foreign_key_count = 0 THEN
    ALTER TABLE students ADD CONSTRAINT students_agent_fk FOREIGN KEY (agent_id) REFERENCES agents(id) ON DELETE SET NULL;
  END IF;
