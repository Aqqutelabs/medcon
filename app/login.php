...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['_csrf'] ?? '')) $error = 'Invalid form submission.';
        ...
        if ($email === '' || $password === '') $error = 'Invalid credentials.';
              ...
              $stmt = $pdo->prepare("SELECT u.*, a.approval_status FROM users u LEFT JOIN agents a ON a.user_id=u.id WHERE u.email = ? LIMIT 1"); $stmt->execute([$email]); $user = $stmt->fetch();
...
<form method="post" class="mc-auth-form" novalidate>
    ...
    <label><span>Password</span><div class="mc-password-wrapper"><input type="password" name="password" autocomplete="current-password" required><button type="button" class="mc-password-toggle" aria-label="Show password" data-show="false">👁</button></div></label>
