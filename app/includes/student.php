...
function student_portal_start(string $title, string $active, array $student): void
{
    $nav = [
        ...
        'eligibility' => [app_url('student/eligibility.php'), 'Eligibility', 'file'],
...
{
    echo '</main><footer class="mc-portal-footer"><span>&copy; ' . date('Y') . ' Medcon Educational Services and Consultancy Limited</span><span>Crafted by <a href="https://aqqute.com" target="_blank" rel="noopener noreferrer">Aqqute</a></span></footer></div></div><script>(function(){var button=document.querySelector(".mc-menu-toggle"),sidebar=document.querySelector(".mc-portal-sidebar");if(!button||!sidebar)return;button.addEventListener("click",function(){var open=document.body.classList.toggle("mc-nav-open");button.setAttribute("aria-expanded",open?"true":"false");});document.addEventListener("keydown",function(event){if(event.key==="Escape"){document.body.classList.remove("mc-nav-open");button.setAttribute("aria-expanded","false");}});})();</script></body></html>';
}
